...
browsing movies and sharing watch rooms with named users.
...
| `ROOMS_FILE`  | `./data/current.json` | Active room state          |
...
2. Pick a movie — the server creates a watch room (persisted to
   `data/current.json`) and redirects you to `/room/:roomId`.
3. Share the room URL with someone else. Anyone who opens the same URL is
   asked for their name before joining the room.
4. Move the mouse over the video (or tap it on mobile) to reveal the room
   overlay: movie title, host/viewer status, your name, other users in the
   room, the room ID, and a copy-link button.

The user who creates a room becomes its host; other visitors join by entering
a name. There is no authentication — every visitor is tracked with a unique
anonymous `pw_session` cookie (never by IP address). Rooms persist in
`data/current.json` while the server runs.
...
Steps 1–3 only: local video streaming, the movie catalog + dark UI, and
file-backed watch rooms with named users. WebSockets, playback
synchronization, chat, and a database are not implemented yet.
